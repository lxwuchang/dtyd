...
public class Excel2007WriterZhipaoOutcomeYearByMonth {
	...
	public void createSheet(String sheetname) {
		...
		if (sheetname.equals("汇总")) {
			...
			sh.setColumnWidth((short) 7, (short) 2800);
			sh.setColumnWidth((short) 8, (short) 2800);
			sh.setColumnWidth((short) 9, (short) 2800);
		}
		flag = 0;
	}
	...
	public void process(String filename, List dataList, int startnum, int total)
		...
		for (int rownum = 0; rownum < len; rownum++) {
			ZhipaoOutcome b = (ZhipaoOutcome) dataList.get(rownum);
			Row row = sh.createRow(startnum + rownum + 1);

			ttransformprice = ttransformprice + b.getShouldincome();
			...
			cell = row.createCell(4);
			address = new CellReference(cell).formatAsString();
			cell.setCellValue(b.getJipaoweight());
			cell.setCellStyle(style);

			cell = row.createCell(5);
			address = new CellReference(cell).formatAsString();
			cell.setCellValue(b.getTotalweight());
			cell.setCellStyle(style);

			cell = row.createCell(6);
			address = new CellReference(cell).formatAsString();
			cell.setCellValue(b.getShouldincome());
			cell.setCellStyle(style);

			cell = row.createCell(7);
			address = new CellReference(cell).formatAsString();
			cell.setCellValue(b.getTuikuan());
			cell.setCellStyle(style);

			cell = row.createCell(8);
			address = new CellReference(cell).formatAsString();
			cell.setCellValue(b.getRealincome());
			cell.setCellStyle(style);
