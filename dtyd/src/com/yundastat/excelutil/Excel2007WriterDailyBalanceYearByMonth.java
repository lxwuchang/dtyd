...
public class Excel2007WriterDailyBalanceYearByMonth {
	...
	public void createSheet(){
		...
		sh.setColumnWidth((short)1,(short)1500);
    ...
    public void process(String filename,List dataList,int startnum,int total) throws Exception 
    {
        ...
        if(dataList!=null&&dataList.size()>0)
    	 ...
    	 if(total==mnum){
			...
			// 保留2位小数
   	 	     tjijian = bb.setScale(2, BigDecimal.ROUND_HALF_UP)
					.doubleValue();
			...
			// 保留2位小数
             ...
             bb = new BigDecimal(tbuzhufei);
   			// 保留2位小数
              tbuzhufei = bb.setScale(2, BigDecimal.ROUND_HALF_UP)
   					.doubleValue();
              cell = row.createCell(4);  
              address = new CellReference(cell).formatAsString();
 			...
 			// 保留2位小数
             tfakuan = bb.setScale(2, BigDecimal.ROUND_HALF_UP)
 					.doubleValue();
             cell = row.createCell(6);  
             address = new CellReference(cell).formatAsString();
             ...
             bb = new BigDecimal(tpaisong);
  			// 保留2位小数
             tpaisong = bb.setScale(2, BigDecimal.ROUND_HALF_UP)
  					.doubleValue();
              cell = row.createCell(7);  
              address = new CellReference(cell).formatAsString();
             ...
             bb = new BigDecimal(ttransformprice+tbuzhufei+tweightprice+tjijian+tfakuan+tpaisong+tzhuanjian+tdiujian);
 			// 保留2位小数
           whole = bb.setScale(2, BigDecimal.ROUND_HALF_UP)
 					.doubleValue();
          cell = row.createCell(10);  
          address = new CellReference(cell).formatAsString();
